...
def phy_2_nwb(unit_spike_times):
    ...
    for u in unit_spike_times.keys():
        nwb[u] = dict(
            [
                ("timestamps", np.array(unit_spike_times[u])),
...
def fill_waveforms(phy_2_mat, raw_waves):
    print(
        f"appending waveforms to struct {len(phy_2_mat['Unit']['mWave'][0])} {raw_waves.keys()}"
    )
    u = 68
...
def _validate(
    units_mat, condition_str, valid_result, message="Invalid export data structure"
):
...
def validate_units_data(units_mat, header=False):
    ...
    _validate(units_mat, "units_mat['Unit'].dtype", [("ts", "O"), ("mWave", "O")])
    ...
    _validate(
        units_mat, "units_mat['Unit']['mWave'].shape[0]", 1
...
def main(phy_dir, med64_bin_path=None, export_filename="export_phy_2_nwb.mat"):
    ...
    if med64_bin_path is not None:
        ...
        raw_waves = get_raw_phy_spike_waves(
            matrix_data, unit_spike_times, unit_list=clust_chan, sample_window_width=61
        )
    ...
    temp_unit = 2
    print(
        f"peak at waveforms: {phy_2_mat['Unit']['mWave'][0].shape} \n"
