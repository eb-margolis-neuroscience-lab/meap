...
D3_DATAFILE_PREFIX = 'd3_cum_fr_10K_interp_debug_'
...
#         print(i)
        ...
        prev_seg = segments[0]
        is_gap = False
        print("groups_with_gaps() --> len(segments): " + str(len(segments)))
        print("groups_with_gaps() --> len(spikes): " + str(len(spikes)))
...
def get_interp_points_df(df_downsampled, interp_units):
    ...
    interp_points_for_df = np.array([])

    for unit, tx in interp_units:
        print("get_interp_points_df() --> Unit " + str(unit) + " " + tx)
    ...
    #     line_euclidist = euclidist(df_line[['cum_dist', 'x_plot']].to_numpy())
        ...
        #     print(base_row.values)
            ...
            if (interp_points_for_df.shape[0] == 0):
    ...
    print("get_interp_points_df() --> is None: " + str(interp_points_for_df))
    print("total interpolated points " + str(interp_points_for_df.shape))
    # if interp_points_for_df.shape
...
def export_d3_data(df_d3, expt_id):
    save_csv_filepath_dir = SAVE_CSV_FILEPATH_DIR #'/Users/walter/Data/margolis/observable/'
    d3_datafile_prefix = D3_DATAFILE_PREFIX #'d3_cum_fr_10K_interp_'
#     d3_datafile_prefix = 'qq_for_d3_10K_interp_'
    save_csv_filepath = save_csv_filepath_dir + d3_datafile_prefix + expt_id + '.csv'
...
def file_attachment_list(expt_list):
    """This function creates and output to cut and paste into an observable notebook
    as part of the drop down file selection menu
    """
#     save_csv_filepath_dir = '/Users/walter/Data/margolis/observable/'
    attach_list = "["
...
def main_old():
    """
    This was the main() function in the 2021 version of the code.
    Deprecated
    """
    max_d3_rows = 10000 # local constant used in num_new_points()
    SAVE_CSV_FILEPATH_DIR = '/Users/walter/Data/margolis/observable/'
    ...
    export_from_filelist(expt_list[:1], datafile_args)
    print(file_attachment_list(expt_list))
    
    if False:
        """
        Some older debugging commands from the 2021 version
        Deprecated
        """
        # %%debug
        expt_id = expt_list[0]
