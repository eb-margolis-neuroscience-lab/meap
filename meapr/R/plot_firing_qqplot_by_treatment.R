...
#'
#'  If the counts occur independently through time as is assumed in a Poisson
#'  process model then for each neuron, the line be diagonal (following the
#'  guide blue line)
#'    * If there is some lag to reach a stead state then the lines will look
#'      like they have a kink in it
...
plot_firing_qqplot_by_treatment <- function(
  ...
  if (!is.null(highlight_units)) {
    p <- p +
      ggplot2::geom_line(
        data = data |> dplyr::filter(neuron_index %in% highlight_units),
        mapping = ggplot2::aes(
          x = (time_step - begin) / (end - begin),
          y = cum_dist,
          group = neuron_index),
  ...
  cat("creating output directory '", output_base, "'\n", sep = "")
    ...
    if (verbose) {
      cat(
  "Saving firing_qqplot_by_treatment  plot for experiment ",
