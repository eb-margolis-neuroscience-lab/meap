...
pseudo_count <- 1
exposure_counts_pseudo_count <- experiment$treatments |>
  purrr::pmap_dfr(function(treatment, begin, end){
    tibble::tibble(
      treatment = treatment,
      neuron_index = seq_len(n_neurons),
      pad_count = pseudo_count,
      exposure = end - begin)
  }) |>
  dplyr::left_join(
    experiment$firing |>
...
# https://github.com/paul-buerkner/brms/issues/146 suggests
# adding offset(log(n))
#
# Attempt at trying to model the fact that individual neurons may behave
# differently, yet consistently across conditions
...
summary(fit_poisson)
summary(fit_poisson) |>
  utils::capture.output(file = "product/demo_fit_poisson_summary_190514.txt")
brms::posterior_summary(fit_poisson)
brms::marginal_effects(fit_poisson)
ggplot2::ggsave(
  "product/demo_fit_poisson_marginal_effects_190514.pdf",
  width = 6,
  height = 6)
ggplot2::ggsave(
  "product/demo_fit_poisson_marginal_effects_190514.png",
  width = 6,
  height = 6)
