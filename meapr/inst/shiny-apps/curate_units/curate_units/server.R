...
shinyServer(function(input, output) {
    ...
    output$plot_waveform_lattice <- renderPlot({
        ...
        p <- experiment |>
          meapr::plot_waveform_lattice(output_base = NULL)
        p
    })
    output$plot_waveform_correlation_matrix <- renderPlot({
        load(input$experiment_path)
        p <- experiment |>
          meapr::plot_waveform_correlation_matrix(output_base = NULL)
        p
    })
    ...
    output$plot_firing_density_by_neuron <- renderPlot({
        load(input$experiment_path)
        p <- experiment |>
          meapr::plot_firing_density_by_neuron(output_base = NULL)
        p
    })
    ...
    output$plot_firing_rate_by_neuron <- renderPlot({
        load(input$experiment_path)
        p <- experiment |>
          meapr::plot_firing_rate_by_neuron(output_base = NULL)
        p
    })
    output$plot_firing_rate_by_treatment <- renderPlot({
        load(input$experiment_path)
        p <- experiment |>
          meapr::plot_firing_rate_by_treatment(output_base = NULL)
        p
    })
    output$plot_firing_qqplot_by_treatment <- renderPlot({
        load(input$experiment_path)
        p <- experiment |> meapr::plot_firing_qqplot_by_treatment(
